...
def main(args, env_name, number, seed):
    ...
    while total_steps < args.max_train_steps:
        ...
        while not done:
            ...
            if total_steps % args.evaluate_freq == 0 or total_steps == 1:
                print('Evaluating...')
                evaluate_num += 1
                ...
                if best_evaluate_reward < evaluate_reward:
                    best_evaluate_reward = evaluate_reward
                    torch.save(agent.actor.state_dict(), 'output_ckpt/PPO_discrete_env_{}_number_{}_seed_{}_actor.ckpt'.format(env_name, number, seed))
                    torch.save(agent.critic.state_dict(), 'output_ckpt/PPO_discrete_env_{}_number_{}_seed_{}_critic.ckpt'.format(env_name, number, seed))
                    print("Update checkpoints!!! evaluate_num:{} \t best_evaluate_reward:{} \t".format(evaluate_num, best_evaluate_reward))
        data_index += 1


if __name__ == '__main__':
    parser = argparse.ArgumentParser("Hyperparameter Setting for PPO-discrete")
    parser.add_argument("--max_train_steps", type=int, default=int(204800), help=" Maximum number of training steps 2e5")
    parser.add_argument("--evaluate_freq", type=float, default=4096, help="Evaluate the policy every 'evaluate_freq' steps 5000")  # 1024
    parser.add_argument("--save_freq", type=int, default=5, help="Save frequency")  # 5
    parser.add_argument("--batch_size", type=int, default=4096, help="Batch size 2048")  # 1024
    parser.add_argument("--mini_batch_size", type=int, default=128, help="Minibatch size 64")  # 32
    ...
    parser.add_argument("--K_epochs", type=int, default=128, help="PPO parameter")
    ...
    parser.add_argument('--train_dataset', default="gsm8k", type=str, choices=['gsm8k-200-eval', 'gsm8k', 'SVAMP', 'MultiArith', 'temp_5'])
    ...
    parser.add_argument('--eval_path', default="", type=str, help='if not none, continuous training')
    # parser.add_argument('--actor_model_path', default="", type=str, help='if not none, continuous training')
    # parser.add_argument('--critic_model_path', default="", type=str, help='if not none, continuous training')
