...
class PPO_discrete:
    def __init__(self, args):
        ...
        if args.eval_path != '':
            self.actor.load_state_dict(torch.load(args.eval_path+'_actor.ckpt'))
            self.critic.load_state_dict(torch.load(args.eval_path+'_critic.ckpt'))
            print('Load Checkpoints Finish!!!')
    ...
    def evaluate(self, s):  # When evaluating the policy, we select the action with the highest probability
        ...
        a_prob = self.actor(s).cpu().detach().numpy().flatten()
        a = np.argmax(a_prob)
        return [a]
    ...
    def choose_action(self, s):
        ...
        with torch.no_grad():
            prob = self.actor(s)
            if self.args.np_sample:
                a = np.random.choice(self.args.n_actions, p=prob[0].cpu().numpy())
                a_logprob = torch.log(prob[0][a])
                return [a], [a_logprob.cpu().numpy()]
            else:
                dist = Categorical(probs=prob[0])
                a = dist.sample()
